...
<div class="max-w-none mx-auto bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 flex">
    <div class="p-5">
        ...
        <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">Silahkan lengkapi profil Anda dengan data yang valid.</p>
...
<form  action="/edit" method="POST" enctype="multipart/form-data">
    ...
    <div class="relative z-0 w-full mb-6 group">
        <input type="text" name="name" id="namaLengkap" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " value="{{ auth()->user()->name }}" required autocomplete="off"/>
    ...
    <div class="relative z-0 w-full mb-6 group">
        <input type="text" name="address" id="alamat" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " value="{{ auth()->user()->jobSeeker->address }}"  required autocomplete="off"/>
    ...
    <div class="relative z-0 w-full mb-6 group">
        <input type="text" name="phone_number" id="phone" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"  pattern="[0-9]+" title="Nomor telepon hanya boleh berisi angka" placeholder=" "  value="{{ auth()->user()->jobSeeker->phone_number }}" required  autocomplete="off"/>
        <label for="phone" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6" >Nomor Telephone</label>
    </div>
    <div class="relative z-0 w-full mb-6 group">
    ...
    <a href="{{ url ('/dashboard') }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Kembali</a>
  </form>
  @if (session('success'))
